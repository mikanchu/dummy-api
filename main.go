...
type Joke struct {
	Text string `json:"joke,omitempty"`
}

var all_jokes []Joke

func GetJokes(c *gin.Context) {
	// json.NewEncoder(w).Encode(all_jokes)
	c.JSON(http.StatusOK, gin.H{"status": http.StatusOK, "data": all_jokes})
}
func GetJoke(c *gin.Context) {
	index := rand.Intn(len(all_jokes))
	// json.NewEncoder(w).Encode(all_jokes[index])
	c.JSON(http.StatusOK, gin.H{"status": http.StatusOK, "data": all_jokes[index]})
}

func PopulateJokes() {
	local_data := [607]string{
	...
	for _, element := range local_data {
		all_jokes = append(all_jokes, Joke{Text: element})
	}
}
